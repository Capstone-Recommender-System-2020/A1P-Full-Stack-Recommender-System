...
class get_events_1st_alg(APIView):
    def get(self, request, format=None):
        ...
        Ratings = pd.read_sql_query(
            '''SELECT * FROM arts_user_interaction;''', engine)

        Mean = Ratings.groupby(by="User_ID", as_index=False)[
            'User Rating'].mean()
        Rating_avg = pd.merge(Ratings, Mean, on="User_ID")
        Rating_avg['adg_rating'] = Rating_avg['User Rating_x'] - \
            Rating_avg['User Rating_y']

        check = pd.pivot_table(Rating_avg, values='User Rating_x',
                               index='User_ID', columns='Event_ID')
        ...
        Movie_user = Rating_avg.groupby(
            by='User_ID')['Event_ID'].apply(lambda x: ','.join(x))
        ...
        def User_item_score1(user):
            ...
            Movies_under_consideration = list(
                map(str, Movies_under_consideration))
            score = []
            for item in Movies_under_consideration:
                c = final_event.loc[:, item]
                d = c[c.index.isin(b)]
                f = d[d.notnull()]
                avg_user = Mean.loc[Mean['User_ID']
                                    == user, 'User Rating'].values[0]
            ...
            data = pd.DataFrame(
                {'Event_ID': Movies_under_consideration, 'score': score})
            top_5_recommendation = data.sort_values(
                by='score', ascending=False).head(5)
            Movie_Name = top_5_recommendation.merge(
                events, how='inner', on='Event_ID')
...
class get_events_2nd_alg(APIView):
    def get(self, request, format=None):
        ...
        df_existingUser = pd.read_sql_query(
            '''SELECT * FROM arts_user_interaction;''', engine)
        df = pd.read_sql_query(
            '''SELECT * FROM arts_user_interaction;''', engine)
        df_NewUser_SurveyFeed = pd.read_sql_query(
            '''SELECT * FROM arts_new_user''', engine)
        ...
        df_existingUser['User Rating'] = df_existingUser['User Rating'].apply(
            literal_eval)
        ...
        NewUser_SurveyOutput = pd.DataFrame(
            columns=['New_User_ID', 'Genre 1', 'Genre 2', 'Genre 3'])
        ...
        for i in range(df_NewUser_SurveyFeed.shape[0]):
            ...
            Genre1 = newuser_thumbsup.loc[newuser_thumbsup['Genre']
                                          == df_NewUser_SurveyFeed["Genre 1"][i]]
            Genre2 = newuser_thumbsup.loc[newuser_thumbsup['Genre']
                                          == df_NewUser_SurveyFeed["Genre 2"][i]]
            Genre3 = newuser_thumbsup.loc[newuser_thumbsup['Genre']
                                          == df_NewUser_SurveyFeed["Genre 3"][i]]
        ...
        existing_user_analysis = pd.DataFrame(
            columns=["User_ID", "Genre 1", "Genre 2", "Genre 3"])
        ...
        for user in user_list:
            # 1: store user 1 events that were thumbed up
            user_df = thumbsup_df.loc[thumbsup_df["User_ID"] == user]
            # 1: user 1 attended event's unique genre
            user_genre = list(user_df["Event Genre"].unique())
            result = list(
                (Counter(Genre_list) - Counter(user_genre)).elements())
            print("User", user)
            print("------------")
            print(Genre_list)
            print(user_genre)
            # Show genres not attended and also given thumbs down/neutral
            print(result)
            # print(result)
            ...
            if(len(result) > 2):   # for 3 or more genres not seen/explored
                ...
                for genre in result:
                    ...
                    top_3 = sub_genre_df["Event Watched"].value_counts(
                    ).index[0:3]
                    genre_dict[genre] = top_3
                    total_dict.append(genre_dict)
                    # print(total_dict)
                    # print(total_dict[0])
                # Top 3 events generated for all unexplored genres
                print(total_dict)
                # Randomly selecting 3 genres for more than 3 genres
                three_genres = random.sample(total_dict, k=3)
                print(three_genres)
                new_row = {"User_ID": user, "Genre 1": three_genres[0],
                           "Genre 2": three_genres[1], "Genre 3": three_genres[2]}
                existing_user_analysis = existing_user_analysis.append(
                ...
                for genre in result:
                    ...
                    if(genre == result[0]):
                        top_4 = sub_genre_df["Event Watched"].value_counts(
                        ).index[0:4]
                        genre_dict[genre] = top_4
                        total_dict.append(genre_dict)
                    else:
                        top_5 = sub_genre_df["Event Watched"].value_counts(
                        ).index[0:5]
                        genre_dict[genre] = top_5
                        total_dict.append(genre_dict)

                new_row = {"User_ID": user,
                           "Genre 1": total_dict[0], "Genre 2": total_dict[1]}
                existing_user_analysis = existing_user_analysis.append(
                    new_row, ignore_index=True)
...
class get_events_3rd_alg(APIView):
    def get(self, request, format=None):
        ...
        arts_user_interaction = pd.read_sql_query(
            '''SELECT * FROM arts_user_interaction;''', engine)

        user_id_final = arts_user_interaction.iloc[9::10, 1:3]

        def get_event_from_user(user_id):
            event_from_user = user_id_final[user_id_final['User_ID']
                                            == user_id]['Event_ID']
        ...
        arts_event['array_categories'] = arts_event.apply(
            array_creation, axis=1)
        ...
        indices = indices = pd.Series(
            arts_event.index, index=arts_event['Event_ID'])
        ...
        for event in event_id_1stmethod:
            ...
            event_name_final_1stmethod.append(
                arts_event[arts_event['Event_ID'] == event]['Name'].values[0])

        for score in similarity_scores_1stmethod:
            similarity_scores_final_1stmethod.append(score[1])

        first_method_df = pd.DataFrame(list(zip(event_id_final_1stmethod, event_name_final_1stmethod,
                                                similarity_scores_final_1stmethod)), columns=['Event_ID', 'Event_Name', 'Similarity_Score'])

        for event_info in list(zip(event_id_final_1stmethod, event_name_final_1stmethod, similarity_scores_final_1stmethod)):
            event_score_list_1stmethod.append(
                {'Event_ID': event_info[0], 'Event_Name': event_info[1], 'Similarity_Score': event_info[2]})
            ...
            for i in range(len(arts_data.index)):
                ...
                if event.id == event_id:
                    while j < len(arts_data.index):
                        ...
                        event.similarity_scores[arts_data.loc[j][1]] = (
                            sim_attributes)
                        j += 1
        ...
        for event in unique_event_id_1st_and_2nd_method:
            ...
            event_name_final_cumulative.append(
                arts_event[arts_event['Event_ID'] == event]['Name'].values[0])
            event_method_count_cumulative.append(
                frequency_of_similar_events[event])
            ...
            if (event in event_id_1stmethod) and (event in event_id_2ndmethod):
                ...
                event_algorithm_and_score_final_cumulative.append(
                    {'1st method': first_method_df.loc[first_method_df['Event_ID'] == event, 'Similarity_Score'].iloc[0]})
        ...
        for event_info in list(zip(event_id_final_cumulative, event_name_final_cumulative, event_method_count_cumulative, event_algorithm_and_score_final_cumulative)):
            event_score_list_cumulative.append({'Event_ID': event_info[0], 'Event_Name': event_info[1],
                                                'Number_Of_Methods_That_Outputted_This_Event': event_info[2], 'Similarity_Score': event_info[3]})
